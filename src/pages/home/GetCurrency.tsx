...
const GetCurrency: React.FC<propType> = ({ tags, giveCurrency, setGetCurrency }: propType) => {
  ...
  const onChangeCurrency = (value: number) => {
    setCurrency(value)
    if (active?.name) {
      setGetCurrency({ name: active.name, value, type: active.type })
    }
  }
  ...
  const onChangeItem = (item: Item) => {
    setActive(item)
    if (item.name) {
      setGetCurrency({ name: item.name || item.type, value: currency, type: item.type })
    }
  }
