...
const GetCurrency: React.FC<propType> = ({
	...
	const [list, setList] = useState(directions)
	...
	useEffect(() => {
		setList(directions.filter((item: any) => item.type !== giveCurrency.type))
	}, [giveCurrency])
	...
	const onSearchType = (type: string) => {
		...
		if (type === 'all') {
			...
			setList(directions.filter((item: Item) => item.type === type))
		}
	}
